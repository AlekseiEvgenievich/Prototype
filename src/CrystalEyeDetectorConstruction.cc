...
G4VPhysicalVolume* CrystalEyeDetectorConstruction::ConstructPayload()
{
    ...
    G4VisAttributes* VisAtt_sp= new G4VisAttributes();
	VisAtt_sp->SetVisibility(true);
	VisAtt_sp->SetForceSolid(true);
	VisAtt_sp->SetColour(G4Colour::Cyan());


/*   G4double innerRadius = 17.99*cm;
G4double outerRadius = 18.01*cm;
G4double startPhi = 0.*deg;
G4double endPhi = 180.*deg;
G4double startTheta = 0.*deg;
G4double endTheta = 90.*deg;

G4Sphere* innerSphere = new G4Sphere("InnerSphere",
                                     0.*cm, innerRadius,
                                     startPhi, endPhi,
                                     startTheta, endTheta);

G4Sphere* outerSphere = new G4Sphere("OuterSphere",
                                     0.*cm, outerRadius,
                                     startPhi, endPhi,
                                     startTheta, endTheta);

G4SubtractionSolid* sphericalLayer = new G4SubtractionSolid("SphericalLayer",
    ...
    G4LogicalVolume* logicalVolumesp = new G4LogicalVolume(sphericalLayer, defaultMaterial, "SphereLogical");


    new G4PVPlacement(0,                   // no rotation
                  G4ThreeVector(),     // at (0,0,0)
                  logicalVolumesp,       // its logical volume
                  "Sphere",            // its name
	...
	logicalVolumesp->SetVisAttributes(VisAtt_sp);*/
    ...
    // for (int i=91; i<92; i++) {/DetectorStructure/Structure_V1/Cal
		file_name.clear();
...
/*		for (int i=0; i<1; i++) {
    //  for (int i=91; i<92; i++) {
		...
		logicACD = new G4LogicalVolume(
				solidACD,
				defaultMaterial,
				"ACD"+std::to_string(i+1),
				0, 0, 0);
		logicACD->SetVisAttributes(VisAtt_ACD);
		physiACD = new G4PVPlacement(
				0,
				G4ThreeVector(0*cm, 3*cm, 1.3*cm),
				"ACD"+std::to_string(i+1),
				logicACD,
				physiWorld,
				false,
