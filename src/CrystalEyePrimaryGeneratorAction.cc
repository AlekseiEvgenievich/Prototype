...
/*double CrystalEyePrimaryGeneratorAction::Func_DiffuseGamma(double eng) {
    double f;
    if(eng <= 60.0) {
        f = 7.877 * std::pow(eng, -1.29) * std::exp(-eng / 41.13);
    } else {
        f = 4.32e-4 * std::pow(eng / 60.0, -6.5)
            + 8.4e-3 * std::pow(eng / 60.0, -2.58)
            + 4.8e-4 * std::pow(eng / 60.0, -2.05);
    }
    return f;
}

// Function to generate a particle with energy according to Func_DiffuseGamma
double CrystalEyePrimaryGeneratorAction::GenerateParticleEnergy() {
    double eng_min = 1.0; // Minimum energy in keV
    double eng_max = 100000.0; // Maximum energy in keV
    double max_pdf = 8; // Calculate the maximum value of Func_DiffuseGamma in the range

    while (true) {
        double eng = G4UniformRand() * (eng_max - eng_min) + eng_min; // Random energy
        double pdf_val = Func_DiffuseGamma(eng);
        if (G4UniformRand() * max_pdf < pdf_val) {
            return eng * keV; // Convert to Geant4 units and return
        }
    }
}


void CrystalEyePrimaryGeneratorAction::calculateFuncDiffuseGamma() {

    double totalSum = 0.0;
    double startEnergy = 0.1; // 0.1 keV
    double endEnergy = 2000.0; // 2 MeV
    double logStepFactor = 1.1; // Log step factor

    // Populate the map
    for (double energy = startEnergy; energy <= endEnergy; energy *= logStepFactor) {
        double value = Func_DiffuseGamma(energy);
        energyToValueMap[energy] = value;
        totalSum += value;
    }

    // Normalize the values and store them with their keys
    for (const auto &pair : energyToValueMap) {
        double normalizedValue = pair.second / totalSum;
...
void CrystalEyePrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent) {
    // Generate energy using the Func_DiffuseGamma distribution
    double energy = GenerateParticleEnergy();
    fParticleGun->SetParticleEnergy(energy);

    // Generate random position on the sphere's surface for uniform isotropic distribution
    //double theta = std::acos(2 * G4UniformRand() - 1); // Polar angle for uniform distribution
    double theta = std::acos(G4UniformRand());
    double phi = 2.0 * M_PI * G4UniformRand(); // Azimuthal angle uniformly distributed
    ...
    // Set particle momentum direction to be inward towards the center of the sphere
    G4ThreeVector direction = -G4ThreeVector(x, y, z).unit();
...
/*void CrystalEyePrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent) {
    ...
    // Generate random direction within the upper hemisphere
    double theta = std::acos(G4UniformRand()); // Polar angle between 0 and π/2
    double phi = 2.0 * M_PI * G4UniformRand(); // Azimuthal angle between 0 and 2π

    fParticleGun->SetParticleMomentumDirection(G4ThreeVector(std::sin(theta) * std::cos(phi), 
                                                            std::sin(theta) * std::sin(phi), 
                                                            std::cos(theta)));

    // Set particle position on a sphere of radius 7 cm
    double radius = 20.0 * cm;
    double x = radius * std::sin(theta) * std::cos(phi);
    double y = radius * std::sin(theta) * std::sin(phi);
    double z = radius * std::cos(theta);
